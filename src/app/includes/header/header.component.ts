  ...
  setupFirebaseListener() {
    try {
      ...
      onMessage(messaging, (payload: any) => {
        ...
        // Add the notification to the top of the list in real-time
        const newNotif = {
          id: Date.now(),
  ...
  fetchNotifications() {
    ...
    this.webapi.getNotifications({ userId: this.loginId }).subscribe((res: any) => {
      if (res.status === 'success' && res.data) {
        ...
        this.unreadCount = this.notifications.filter((n: any) => n.is_read === '0' || n.is_read === 0).length;
      }
    });
  }
    ...
    if (this.notificationOpen) {
      ...
      if (this.unreadCount > 0) {
        this.webapi.markNotificationsRead({ userId: this.loginId }).subscribe(() => {
          this.unreadCount = 0;
          this.notifications.forEach((n: any) => n.is_read = '1');
        });
      }
    }
  }
